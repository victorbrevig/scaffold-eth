...
contract SVGgenerator {
    ...
    function renderTokenBodyById(uint8 tier) pure external returns (string memory) { 
        // Body
        if(tier==0){
    ...
    function renderTokenHatById(uint8 hat) pure external returns (string memory) {
    ...
    // Face
  function renderTokenFaceById(uint8 face) pure external returns (string memory) {
    if(face==0){ // default
      return string(abi.encodePacked(
      '<g>',
      ...
      '<line x1="736.91" y1="569" x2="736.91" y2="600" /></g>'
      ));
    }
