...
contract YourCollectible is ERC721Enumerable, Ownable {
    ...
    uint8 constant noOfHats = 20;
    uint8 constant noOfEyes = 17;
    uint8 constant noOfMouths = 17;
    uint8 constant noOfFullFaces = 7;
  ...
  */
    ...
    function getBlooper(uint256 _id) public view returns (Blooper memory) {
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        ...
        string memory name = string(
            abi.encodePacked("Blooper #", id.toString())
        );
        string memory description = string(
            abi.encodePacked(
                "This Blooper is the color #",
        ...
        returns (string memory)
    {
        ...
        if (idToBlooper[id].mode == 1) {
            //transparent
            svgMode = ' fill-opacity="0.3" ';
        } else if (idToBlooper[id].mode == 2) {
            //glow
        ...
        string memory svgP1 = string(
            abi.encodePacked(
                ...
                cols[idToBlooper[id].gradientColor1],
                '" />',
                '<stop offset="1" stop-color="#',
                cols[idToBlooper[id].gradientColor2],
                '" /></linearGradient>'
            )
        );
        ...
        string memory svgP2 = string(
            abi.encodePacked(
                ...
                extraGenerator.render(
                    idToBlooper[id].extra,
                    cols[idToBlooper[id].extraColor]
                ),
                '<g fill="#',
                cols[idToBlooper[id].bodyColor],
                '"',
        ...
        string memory svgP3 = string(
            abi.encodePacked(
                fullFaceGenerator.render(
                    idToBlooper[id].fullFace,
                    cols[idToBlooper[id].fullFaceColor]
                ),
                hatGenerator.render(
                    idToBlooper[id].hat,
                    cols[idToBlooper[id].hatColor]
                ),
                eyeGenerator.render(
                    idToBlooper[id].eye,
                    cols[idToBlooper[id].eyeColor]
                )
            )
        );
        ...
        string memory svgP4 = string(
            abi.encodePacked(
                mouthGenerator.render(
                    idToBlooper[id].mouth,
                    cols[idToBlooper[id].mouthColor]
                ),
                maskGenerator.render(
                    idToBlooper[id].mask,
                    cols[idToBlooper[id].maskColor]
                ),
                detailGenerator.render(
                    idToBlooper[id].detail,
                    cols[idToBlooper[id].detailColor]
                ),
                "</g></svg>"
            )
        );
    ...
    function upgrade(uint256 toUpgradeId, uint256 toBurnId) public {
        // Checks
        require(
            _isApprovedOrOwner(msg.sender, toUpgradeId) &&
                _isApprovedOrOwner(msg.sender, toBurnId),
            "NOT OWNER/APPROVED"
        );
        require(
            idToBlooper[toUpgradeId].tier == idToBlooper[toBurnId].tier,
            "NOT SAME TIER"
        );
        ...
        if ((uint8(predictableRandom[1]) % 2) == 0) {
            ...
            idToBlooper[toBurnId].hat == 11
                ? idToBlooper[toUpgradeId].hatColor = idToBlooper[toUpgradeId]
                    .bodyColor
                : idToBlooper[toUpgradeId].hatColor = idToBlooper[toBurnId]
                .hatColor;
        } else {
